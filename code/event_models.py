...
class EventModel:
    ...
    @abstractmethod
    def detect_event(self, x):
        pass


class HiddenMarkovEventModel(EventModel):
    ...
    def detect_event(self, x, epsilon=1.2, delta=3):
        num_time_steps = x.shape[0]

        v = np.zeros((num_time_steps, self.num_hidden_states))  # cumulative likelihoods
        s = [[(0, 0) for _ in range(self.num_hidden_states)] for _ in range(num_time_steps)]  # starting positions
        ...
        for t in range(num_time_steps):
            for i in range(self.num_hidden_states):
                ...
                if t == 0:
                    ...
                    v2 = np.max(v[t - 1] * self.a[:, i]) * self.b(i, x[t]) / epsilon
                    v[t, i] = max(v1, v2)

                # starting position computation
                if v[t, i] == self.pi[i] * self.b(i, x[t]) / epsilon:
                    s[t][i] = (t, i)
                else:
                    j = np.argmax(v[t - 1])
                    s[t][i] = s[t - 1][j]
                ...
                # if v[t, i] >= 1 / epsilon ** delta: TO CLARIFY !!!
                if v[t, i] >= epsilon ** delta:
            ...
            for c in set(candidates):
                if c[1] not in set([s[t][i] for i in range(self.num_hidden_states)]) or t == num_time_steps - 1:
...
class PitchModel(HiddenMarkovEventModel):
